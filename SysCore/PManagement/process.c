...
static void set_process_entry(struct Process *proc, uint64_t addr)
{
    ...
    proc->context = stack_top - sizeof(struct interrupt_Frame) - 7*8;   
    *(uint64_t*)(proc->context + 6*8) = (uint64_t)TrapReturn;

    proc->frame = (struct interrupt_Frame*)(stack_top - sizeof(struct interrupt_Frame)); 
    proc->frame->cs = 0x10|3;
    proc->frame->rip = 0x400000;
    proc->frame->ss = 0x18|3;
    proc->frame->rsp = 0x400000 + PAGE_SIZE;
